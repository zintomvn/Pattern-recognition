...
# Force legacy Keras for compatibility with TF 1.x code in TF 2.16+
...
# Custom batch_norm to replace tf-slim's version and fix naming duplication
@add_arg_scope
def custom_batch_norm(inputs, is_training=False, scope=None, **kwargs):
    import tf_keras
    # Map parameters from slim to keras
    training = kwargs.pop('is_training', is_training)
    # Filter out internal slim arguments
    kwargs = {k: v for k, v in kwargs.items() if not k.startswith('_')}
    # Map momentum
    if 'decay' in kwargs:
        kwargs['momentum'] = kwargs.pop('decay')
    
    # Name handling: Slim's conversion logic can lead to duplication.
    # We use the scope name directly for the Keras layer.
    name = scope if scope else 'BatchNorm'
    
    # Ensure we don't have multiple training arguments
    kwargs.pop('training', None)
    
    # Create and call Keras layer
    # Wrap in a class to provide 'apply' if needed by some slim ops
    class KerasBN(tf_keras.layers.BatchNormalization):
        def apply(self, inputs, training=False):
            return self.__call__(inputs, training=training)
    
    layer = KerasBN(name=name, **kwargs)
    return layer(inputs, training=training)

# Monkeypatch tf_slim
tcl.batch_norm = custom_batch_norm
tcl.layers.batch_norm = custom_batch_norm
