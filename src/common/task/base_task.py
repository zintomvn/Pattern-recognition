...
class BaseTask(object):
    ...
    def __init__(self, cfg, task_dir, logger=None):
        ...
        self.start_epoch = 1  # always initialized; overridden by _resume_model if checkpoint exists

        # CPU mode for --test: override distributed settings and use CPU device
        if getattr(self.cfg, 'test_only', False):
            self.cfg.distributed = 0
            self.cfg.local_rank = 0
        self.device = torch.device("cuda" if torch.cuda.is_available() and not getattr(self.cfg, 'test_only', False) else "cpu")
    ...
    def prepare(self):
        ...
        # In test-only mode skip optimizer, scheduler, losses, and saver
    ...
    def _log_data(self, data, epoch=0, prefix='val', **kwargs):
        ...
        # Skip wandb logging in --test mode (wandb is never initialized)
        if not getattr(self.cfg, 'test_only', False):
